...
void Stage::Push() {
	if (water.is_Push == FALSE && fire.is_Push == FALSE) {
		if (water.y == Rt.y && abs(water.x - Rt.x) <= 60) {			// ºÒ°ú Á¢ÃË
			water.is_Push = TRUE;
		}
		if (fire.y == Rt.y && abs(fire.x - Rt.x) <= 60) {		// ¹°°ú Á¢ÃË
			fire.is_Push = TRUE;
		}
	}
	// Ãæµ¹ Ã¼Å©
...
void Stage::Foot() {
	for (int i = 0; i < 20; ++i) {
		if (Ft[i].W_On) {
			if (Ft[i].x > water.x || (water.x - 60) > (Ft[i].x + Ft[i].wid)) {    // ¹ÛÀ¸·Î ³ª°¬À» °æ¿ì
				bool signal = FALSE;
				for (int beam = water.y; beam < 730; ++beam) {            // ºö ½î±â
					for (int j = 0; j < 20; ++j) {    // ºí·° ÇÏ³ª¾¿ °Ë»ç
		...
		if (Ft[i].F_On) {
			if (Ft[i].x > fire.x || fire.x - 60 > Ft[i].x + Ft[i].wid) {    // ¹ÛÀ¸·Î ³ª°¬À» °æ¿ì
				bool signal = FALSE;
				for (int beam = fire.y; beam < 730; ++beam) {            // ºö ½î±â
					for (int j = 0; j < 20; ++j) {    // ºí·° ÇÏ³ª¾¿ °Ë»ç
						if ((Ft[j].y + average) - beam < 10 && (Ft[j].y + average) - beam > -10) {
		...
		if (water.is_Jumping == TRUE) {
			if (Ft[i].x <= water.x && water.x - 60 <= Ft[i].x + Ft[i].wid) {
				if ((water.y - 70) - (Ft[i].y + Ft[i].hei + average) <= 5 && (water.y - 70) - (Ft[i].y + Ft[i].hei + average) > Ft[i].hei * -1) {    // ¸Ó¸® ºÎ´ÚÄ¡±â
					water.v = 0;
					water.Down = TRUE;
				}
				if (Ft[i].y + average - water.y <= 5 && Ft[i].y + average - water.y > -20) {        // ¾ÈÂø
					water.is_Jumping = FALSE;
					water.Down = FALSE;
					water.v = 50;
